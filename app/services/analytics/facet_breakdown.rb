...
module Analytics
  ...
  class FacetBreakdown
    Row = Struct.new(:label, :amount_cents, :count, keyword_init: true) do
    ...
    Pair = Struct.new(:left, :right, keyword_init: true) do
      ...
      def total             = Money.new(total_cents, "PLN")
    ...
    def self.essential(scope)
      ess = scope.spend.essential
      dis = scope.spend.discretionary
      Pair.new(
        left:  Row.new(label: "Essential",     amount_cents: ess.sum(:amount_cents), count: ess.count),
        right: Row.new(label: "Discretionary", amount_cents: dis.sum(:amount_cents), count: dis.count)
      )
    end
    ...
    def self.recurring(scope)
      rec = scope.spend.recurring
      one = scope.spend.one_off
      Pair.new(
        left:  Row.new(label: "Recurring", amount_cents: rec.sum(:amount_cents), count: rec.count),
        right: Row.new(label: "One-off",   amount_cents: one.sum(:amount_cents), count: one.count)
      )
    end

    # Recurring spend grouped by interval — gives the dashboard a
    # "you'll spend ~X/month on subscriptions" forecast number.
    def self.recurring_by_interval(scope)
      ...
      rows.map { |interval, sum, count|
        Row.new(label: interval || "unknown", amount_cents: sum.to_i, count: count.to_i)
